...
import type { KyberKeyPair, KyberEncapsulation } from "../../wasm-src/kyber-wasm/pkg/kyber_wasm.js";
...
export async function initKyber(): Promise<void> {
  ...
  try {
    ...
    const wasmInit = (await import("../../wasm-src/kyber-wasm/pkg/kyber_wasm.js")).default;
    ...
    if (typeof process !== "undefined" && process.versions?.node) {
      ...
      const wasmFilePath = join(currentDir, "../../wasm-src/kyber-wasm/pkg/kyber_wasm_bg.wasm");
      ...
      // ブラウザ環境の場合
      wasmPath = new URL("../../wasm-src/kyber-wasm/pkg/kyber_wasm_bg.wasm", import.meta.url).href;
    ...
    throw new Error(`Failed to initialize Kyber WebAssembly module: ${error}`);
  }
}
 ...
 * @returns 暗号文と共有秘密
 */
export async function encapsulateKyber(publicKey: Uint8Array): Promise<{
...
export async function decapsulateKyber(
  ...
  const sharedSecret = wasmExports.decapsulate(ciphertext, privateKey, publicKey);
  if (!sharedSecret) {
